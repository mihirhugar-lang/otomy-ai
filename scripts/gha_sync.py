...
def cleanup_residual_balance_artifacts():
    """Remove the retired browser-only residual rows from previously published JSON.

    New books are generated by build_cashbook_view and never create this label. This cleanup keeps
    an older snapshot from resurfacing it when a user selects a range not yet present in R2.
    """
    changed_files = 0
    ...
    def clean(value):
        if isinstance(value, list):
            ...
            changed = False
            for item in value:
                if isinstance(item, dict) and item.get("particulars") == "Verified balance adjustment (residual)":
                    changed = True
                    continue
...
def build_cashbook_view(from_d, to_d, sales, expenses, repayments, opening):
    ...
    movement rows are then reconciled once, at a named physical-count or bank-statement point when
    necessary. There is deliberately no generic ``residual`` row: a stale or incomplete archive
    must be fixed by the sync instead of being hidden in the UI.
    """
    ...
    def _finalize(rows, opening_balance, closing_balance, channel):
        ...
        if abs(gap) > 0.5:
            anchor_date = None
            if channel == "cash":
                applicable = [a for a in overlay.get("anchors", []) if str(a.get("date")) <= str(to_d)]
                if applicable:
                    anchor_date = str(applicable[-1].get("date"))
