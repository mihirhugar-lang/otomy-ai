...
(()=>{
  ...
  const domain=(series,kind)=>{const a=series.flatMap(s=>s.values).map(N);let lo=Math.min(0,...a),hi=Math.max(0,...a);if(kind==='pct')lo=0;if(hi===lo){hi=hi||1;lo=lo===0?0:lo-1;}const p=(hi-lo)*.08;return {lo:lo-p,hi:hi+p};};
  function drawLine(host,rows,series){
    if(!rows.length||!series.some(s=>s.values.some(v=>N(v)!==0))){host.append(E('div',{class:'insight-empty'},'Chart data is not available for this period.'));return;}
    ...
    series.forEach((s,si)=>{const d=s.axis==='right'?rd:ld,pts=s.values.map((v,i)=>[l+i*iw/Math.max(s.values.length-1,1),scale(v,d)]),color=s.color||COLORS[si%COLORS.length],poly=S('polyline',{points:pts.map(p=>p[0].toFixed(1)+','+p[1].toFixed(1)).join(' '),fill:'none',stroke:color,'stroke-width':2.5,'stroke-linejoin':'round','stroke-linecap':'round'});svg.append(poly);pts.forEach((p,i)=>{const c=S('circle',{class:'insight-tip',cx:p[0],cy:p[1],r:s.values.length>90?1.4:2.3,fill:color});c.append(S('title',{},rows[i].date+' · '+s.name+': '+value(s.values[i],s.kind)));svg.append(c);});});
    host.append(svg);
  }
